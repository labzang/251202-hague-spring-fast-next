...
def detect_faces(
    ...
    try:
        ...
        if model_path is None:
            # OpenCV DNN 얼굴 디텍터 모델 다운로드 URL
            # 실제로는 모델 파일이 필요하지만, 여기서는 OpenCV의 기본 얼굴 디텍터 사용
            # 또는 Haar Cascade를 사용하되 confidence는 추정값 사용
            model_path = os.path.join(
                script_dir, "../data/opencv/haarcascade_frontalface_alt.xml"
            )
        ...
        try:
            ...
            if not os.path.exists(model_path):
                raise FileNotFoundError(
                    f"얼굴 디텍션 모델 파일을 찾을 수 없습니다: {model_path}"
                )

            face_cascade = cv2.CascadeClassifier(model_path)
            if face_cascade.empty():
                raise ValueError("얼굴 디텍션 모델을 로드할 수 없습니다.")
        ...
        if save_result:
            ...
            result_dir = image_path_obj.parent
...
if __name__ == "__main__":
    ...
    result = auto_detect_latest_image()
